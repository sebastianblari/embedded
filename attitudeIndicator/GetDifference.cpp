...
//    uint16_t* Coordinates_ptr = DecodeMsgData();
    ...
    if (g_fRollAngle <= M_PI/2 || g_fRollAngle > 3*M_PI/2) {
        for (uint8_t line_index = 0; line_index < 129; line_index++) {
            if(g_u16XYCoordinates[line_index] >= g_u16XYCoordinates_previous[line_index]){
                Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLUE);
                Graphics_drawLine(&g_sContext, line_index, g_u16XYCoordinates[line_index], line_index, g_u16XYCoordinates_previous[line_index]);
            } else if (g_u16XYCoordinates[line_index] < g_u16XYCoordinates_previous[line_index]){
                Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BROWN);
                Graphics_drawLine(&g_sContext, line_index, g_u16XYCoordinates_previous[line_index], line_index, g_u16XYCoordinates[line_index]);
            }
            g_u16XYCoordinates_previous[line_index] = g_u16XYCoordinates[line_index];
        }
